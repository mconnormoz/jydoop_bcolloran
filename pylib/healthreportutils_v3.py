...
class FHRPayload_v3(object):
    ...
    def platform_build_id(self):
        return self.currentAppInfo.get('platformBuildID', None)
    ...
    @property
    def channel(self):
        return self.currentAppInfo.get('updateChannel', 'unknown')

    @property
    def vendor(self):
        return self.currentAppInfo.get('vendor', None)
    ...
    def environments(self):
        return self._o.get('environments', None)

    @property
    def system_info(self):
        return self._o.get('environments', {}).get('current', {}).get('org.mozilla.sysinfo.sysinfo', None)
    ...
    def daily_environ_data(self, reverse=False):
        """Iterate over the daily data in the payload.
        ...
        for day in days:
            for environ in data[day].keys():
                yield day, environ, data[day][environ]
    ...
    def daily_search_counts(self):
        searchesOut=dict()
        for day,data in self.daily_data():
            for environ in data.keys():
                for location in [loc for loc in data[environ].get("org.mozilla.searches.counts",{}).keys() if loc!="_v"]:
                    for engine in data[environ]["org.mozilla.searches.counts"][location].keys():

                        searchesOut[day,location,engine]=searchesOut.get((day,location,engine),0) + data[environ]["org.mozilla.searches.counts"][location][engine]+5

                        # print day,environ,location,engine,data[environ]["org.mozilla.searches.counts"][location][engine]

        return sorted([tuple(list(key)+[count]) for key,count in searchesOut.iteritems()], key=lambda tup: tup[0])
...
class FHRMapper(object):
    ...
    FHRPayload_v3 instance as its 2nd argument instead of the raw payload data.
