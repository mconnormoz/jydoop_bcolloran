...
def reduce(docId, iterOfPartOrJson, context):
    docIdsAndJsonsForThisPart=[]
    for partOrJson in iterOfPartOrJson:
        if type(partOrJson)==tuple: #the part for these docs
            partId = partOrJson[1]
        else: #a record in the part
            docIdsAndJsonsForThisPart.append((docId,partOrJson.strip()))

    for docIdsAndJsons in docIdsAndJsonsForThisPart:
        try:
            context.write(partId,docIdsAndJsons)
