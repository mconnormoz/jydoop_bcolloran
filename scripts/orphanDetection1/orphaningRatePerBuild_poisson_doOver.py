...
def map(key, value, context):
    ...
    #find the build transition dates. these dates are necessarily a subset of the fhrActiveDataDaysList, so going off of that is enough. if there are no days in fhrActiveDataDaysList, then ascribe all activeDaysSinceFhr to the buildIdAt_thisPingDate, and set transition date in to be the fhrActivationDate
    if fhrActiveDataDaysList:
        # appBuildIdTransitions is a list of tuples (buildId, transitionDateIn)
        appBuildIdTransitions=[]
        for day in fhrActiveDataDaysList:
        ...
        # try:
        #     appBuildIdTransitions = [ (payload["data"]["days"][day]["org.mozilla.appInfo.versions"]["appBuildID"][0], day) for day in fhrActiveDataDaysList if ("org.mozilla.appInfo.versions" in payload["data"]["days"][day].keys()) ]

        # except KeyError:
        #     raise Exception( str(payload["data"]["days"][day]["org.mozilla.appInfo.versions"]) )
        ...
        #if there are no days in fhrActiveDataDaysList, then ascribe all activeDaysSinceFhr to the buildIdAt_thisPingDate, and set transition date in to be the fhrActivationDate
        appBuildIdTransitions = [(buildIdAt_thisPingDate,fhrActivationDate)]
