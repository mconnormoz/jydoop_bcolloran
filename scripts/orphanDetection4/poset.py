...
class Poset(object):
    ...
    def __init__(self,nodeList):
        ...
        if not nodeChain[0].prev:
            #nodeChain[0] must not have a prev elt
            self._possibleMinEltIds = set([nodeChain[0].id])
        else:
            raise ValueError
        if not nodeChain[-1].next:
    ...
    def maxElts(self):
        if not self._maxElts:
            self._maxElts = [node for node in self.nodeDict.values() if not node.next]
        return self._maxElts

    def minElts(self):
        if not self._minElts:
            self._minElts =[node for node in self.nodeDict.values() if not node.prev]
    ...
    def _nodeListToChain(self,nodeList):
        if len(nodeList)==0:
            ...
            for i in range(len(nodeList)):
                if i==0:
                    nodeList[i].next.add(nodeList[i+1])
                elif i==len(nodeList)-1:
                    nodeList[i].prev.add(nodeList[i-1])
                else:
                    nodeList[i].next.add(nodeList[i+1])
                    nodeList[i].prev.add(nodeList[i-1])
            return nodeList
    ...
    def addNode(self,node):
        ...
        if len(self.nodeDict.keys())!=len(set(self.nodeDict.keys())):
            print"\nthis node", node
    ...
    def edges(self):
        if not self._edges:
            ...
            for source in self.nodes():
                self._edges += [{"sourceId":source.id,"targetId":target.id,"count":target.nodeCount} for target in source.next]
        return self._edges
        ...
        while queue:
            ...
            for node in thisNode.next:
                if node.childTreeWidth:
                    ...
                    break
            # try:
            #     thisTreeWidth = sum( node.childTreeWidth for node in thisNode.next )
            # except:
            #     thisTreeWidth = None
    ...
    def addWidthOffsets(self):
        ...
        if len(self.minElts())!=1:
            ...
            while(queue):
                ...
                for childNode in thisNode.next:
...
class Node(object):
    ...
    def __init__(self,id,data=None):
        ...
        self.nodeCount = 1
        self.data = data
        self.prev = set() #starts empty, evals to false
        self.next = set() #starts empty, evals to false
        self.childIds = set() #starts empty, evals to false
        self.parentIds = set() #starts empty, evals to false
    def __str__(self):
        return str({"id":self.id,"next":[node.id for node in self.next],"prev":[node.id for node in self.prev]})
    def __repr__(self):
        return str({"id":self.id,"next":[node.id for node in self.next],"prev":[node.id for node in self.prev]})
    def __json__(self):
        return {"id":self.id,"next":[node.id for node in self.next],"prev":[node.id for node in self.prev]}
    def __hash__(self):
        return self.id
    def __eq__(self,other):
        return self.id==other.id
    def mergeNode(self,otherNode):
        self.prev.update(otherNode.prev)
        self.next.update(otherNode.next)
        self.childIds.update(otherNode.childIds)
        self.parentIds.update(otherNode.parentIds)
        self.nodeCount += otherNode.nodeCount
    def children(self):
        return (containingGraph.nodeDict[childId] for childId in self.childIds)
    def parents(self):
        return (containingGraph.nodeDict[parentId] for parentId in self.parentIds)



class DayNode(Node):
    def __init__(self,id,date,minDateInThread,maxDateInThread,data):
        Node.__init__(self,containingGraph,id)
        self.date = date
        # self.widthOffset = None
        # self.nodeCount = 1
        self.data = data
    ...
    def __repr__(self):
        return json.dumps({"id":self.id,"date":self.date,"nodeCount":self.nodeCount,"childWidth":self.childTreeWidth,"widthOffset":self.widthOffset,"maxDateInThread":self.maxDateInThread,"next":[node.id for node in self.next],"prev":[node.id for node in self.prev]})
    def __json__(self):
        return {"id":self.id,"date":self.date,"width":self.nodeCount,"yOffset":self.childTreeWidth}
...
class DayGraph(Poset):
    ...
    def __json__(self):
        {"minDate":self.minDate(),"maxDate":self.maxDate(),"nodes":self.nodes(),"edges":self.edges()}

    def __repr__(self):
        return json.dumps({"minDate":self.minDate(),"maxDate":self.maxDate(),"nodes":self.nodes(),"edges":self.edges()},cls=MyEncoder)
