...
def map(docId, rawJsonIn, context):
    ...
    datePrints = []
    for date in sorted(payload["data"]["days"].keys()):
    ...
    # emit sortedDayInfo
    if datePrints:
        context.getCounter("MAPPER", "(docId,sortedDayInfo) out").increment(1)
        context.write("GRAPH"+docId,tuple(datePrints))
...
def reduce(datePrintOrDocId, docIdIter_OrDayGraphIter, context):
    context.getCounter("REDUCER", "datePrintOrDocId keys in to reducer").increment(1)
    if datePrintOrDocId[0:5]=="GRAPH":
        #in this case, we have a (docId,dayGraphIter)
        ...
        sortedDayInfoList = list(docIdIter_OrDayGraphIter)
        context.write(datePrintOrDocId[5:], sortedDayInfoList[0])

        context.getCounter("REDUCER", "(docId,sortedDayInfo) out").increment(1)
        if 1<len(sortedDayInfoList)<10:
            context.getCounter("REDUCER", "num docIds shared by "+str(len(sortedDayInfoList))+" records").increment(1)
        elif len(sortedDayInfoList)>=10:
        ...
        partIdSet = list(set(docIdIter_OrDayGraphIter))
        if len(partIdSet)==1:
        ...
        linkedDocIds = list(docIdIter_OrDayGraphIter)
